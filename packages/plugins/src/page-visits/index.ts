...
import type { PageVisitsEvent, PageVisitsPlugin, PageVisitsPluginConfig } from './types';
...
export const pageVisitsPlugin: PluginFunction = (plugin, instance, config) => {
  ...
  function increment(): void {
    ...
    // Emit event
    const event: PageVisitsEvent = {
      isFirstVisit: isFirstVisitFlag,
      totalVisits: totalCount,
      sessionVisits: sessionCount,
      firstVisitTime,
      lastVisitTime,
      timestamp: now,
    };
  ...
  function initialize(): void {
    ...
    const respectDNT = config.get('pageVisits.respectDNT') ?? true;
    const autoIncrement = config.get('pageVisits.autoIncrement') ?? true;

    // Check DNT
    if (respectDNT && respectsDNT()) {
